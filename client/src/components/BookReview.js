...
const Review = ({ props }) => {
  ...
  function handleSubmitReview() {
    if (props.eventId && props.bookId && state.user.id)
      fetch(`${SERVER_API}/reviews/create`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          bookId: props.bookId,
  ...
  function handleRemoveReview(id) {
    console.log({ id });
    if (props.eventId && props.bookId && state.user.id)
      fetch(`${SERVER_API}/reviews/remove`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          id,
    ...
    <>
      <div style={{ width: '100%', justifyItems: 'center' }}>
        <Card style={{ width: '75%', border: 0 }}>
          ...
          <textarea style={{ height: '10rem' }} onKeyUp={(e) => setNewReview(e.target.value)} />
          <div style={{ height: '1rem' }} />
          <Button style={{ width: '6rem', alignSelf: 'end' }} onClick={() => handleSubmitReview()}>
            Odeslat
          </Button>
          <div style={{ height: '2rem' }} />
                          ...
                          <Stack direction="vertical">
                            <div style={{ paddingBottom: '6px' }}>
                              <Stack direction="horizontal">
                                <div style={{ color: '#714300', fontWeight: 500 }}>
