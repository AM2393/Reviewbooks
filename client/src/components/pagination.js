...
const Pagination = () => {
    ...
    const handleChangeLimit = (value) => {
        ...
        window.location.replace(`http://localhost:3001/library?page=1&limit=${pageLimit}&filter=%22${urlFilter}%22&search=%22${urlSearch}%22`)
    ...
    function previousPage() {
        if(page > 1) {
            window.location.replace(`http://localhost:3001/library?page=${urlPage - 1}&limit=${urlLimit}&filter=%22${urlFilter}%22&search=%22${urlSearch}%22`)
            changePage(page - 1)
        }
    }
    function nextPage() {
    ...
    function firstPage() {
        if(page <= 1) return;
        window.location.replace(`http://localhost:3001/library?page=1&limit=${urlLimit}&filter=%22${urlFilter}%22&search=%22${urlSearch}%22`)
        changePage(1)
    }
    function lastPage() {
        if(page >= pagesNum) return;
        changePage(pagesNum)
        window.location.replace(`http://localhost:3001/library?page=${pagesNum}&limit=${urlLimit}&filter=%22${urlFilter}%22&search=%22${urlSearch}%22`)
    }

    function showPageNumber(index){
        return Math.abs(page - 1 - index) <= showMax || ((Math.abs(page - 1 - index) - showMax) == 1 && (index == 0 || index == pagesNum - 1))
    }
...
<div className="pagination" >
    <Button variant='secondary' style={{boxShadow: "0px 6px 8px 0px #80808040"}} onClick={() => {
        firstPage()
        }}>
    ...
    <div class="d-flex" style={{width: "275px", justifyContent: "center"}}>
        ...
        {Math.abs(page - 1) > showMax + 1 && <>
            ...
            <Button variant="secondary" style={{boxShadow: "0px 6px 8px 0px #80808050"}} onClick={() => firstPage()}
            >
                {1}
            </Button>
            <div style={{width: "2px"}}/>
        ...
        {Array.from({length: pagesNum}, (_,index) => <>
            {showPageNumber(index) && page == index + 1 && <Button variant="primary" style={{boxShadow: "0px 8px 10px 0px #808080e0"}}>
                {index + 1}
            </Button>}
            {showPageNumber(index) && page != index + 1 && <Button variant="secondary" style={{boxShadow: "0px 6px 8px 0px #80808050"}}
            onClick={() => {
                window.location.replace(`http://localhost:3001/library?page=${index + 1}&limit=${urlLimit}&filter=%22${urlFilter}%22&search=%22${urlSearch}%22`)
                changeUrl(`http://localhost:3001/library?page=${index + 1}&limit=${urlLimit}&filter=%22${urlFilter}%22&search=%22${urlSearch}%22`)
            }}
            >
                {index + 1}
            </Button>}
...
<div className='perPage' style={{position: "fixed", bottom: "130px", right: "10px", justifyItems: "center"}}>
    <Stack direction="vertical" style={{width: "80px", alignItems: "center"}}>
        ...
        <div >
            <input style={{
                width: "50px", height: "40px", borderRadius: "6px",
                borderColor: "#80808080", borderWidth: 2, color: "#404040"
            }}
            value={pageLimit}
