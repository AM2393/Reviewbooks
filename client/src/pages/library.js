...
const DataTest = () => {
  ...
  const { page, limit, contextFilter, changeFilter, contextSearch, changeSearch, defaultLimit } =
  ...
  useEffect(() => {
    fetch(`${SERVER_API}/v1/book/getAllGenres`, {
      method: 'GET',
    }).then(async (response) => {
      ...
      <Container style={{ justifyItems: 'center' }}>
          ...
          <div style={{ width: '100%', position: 'fixed' }}>
            <Card>
              <div style={{ position: 'fixed', width: '100%' }}>
                <h2 className="libraryTitle" libraryTitle>
                  Library
                </h2>
              </div>
              <Card.Body>
                <Stack direction="horizontal">
                        ...
                        <ButtonGroup vertical>
                          {allGenres.map((genre) => (
                            <Button
                              ...
                              onClick={() => setTempFilter(`${tempFilter == `${genre}` ? 'None' : `${genre}`}`)}
                            >
                              <Stack direction="horizontal">{`${genre}`}</Stack>
                            </Button>
                          ))}
                        </ButtonGroup>
                        <Button
                  ...
                  <div
                    className="ms-auto"
                    style={{ display: 'flex', alignItems: 'center', position: 'relative', zIndex: 1000 }}
                  >
                    <ButtonGroup>
                      <Form.Control
                        type="text"
        ...
        <div className="libraryContainer">
          <Row className="centerLibrary" style={{ justifySelf: 'center' }}>
            {displayObjects &&
              displayObjects.map(({ id, title, isbn, cover_url, author, description }) => (
                <div style={{ width: '18rem', margin: '2rem' }}>
                  <Card style={{ boxShadow: '0px 10px 22px -10px #80808080' }}>
                    <Button href={`/book?id=${id}`} variant="light" style={{ borderColor: '#ffffff00' }}>
        ...
        {displayObjects.length == 0 && (
          <div style={{ width: '100vw', height: '75vh', justifyItems: 'center', alignContent: 'center' }}>
            <div
              style={{
                color: '#714300',
                fontSize: '40px',
